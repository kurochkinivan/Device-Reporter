...
[Ссылка на ER-диаграмму](https://dbdiagram.io/d/BIOCAD-69955db3bd82f5fce204b68b). Таблицы:

- `files` (статус обработки файлов)
...
После обработки файла для каждого уникального `unit_guid` генерируется PDF-отчёт в директории `reports_dir`. Файл называется по `unit_guid`, например:

```
output/01749246-95f6-57db-b7c3-2ae0e8be671f.pdf
```

Отчёт содержит карточки для каждого устройства с цветовой кодировкой по классу (`alarm`, `warning`, `working`).

![Report example][readme/report.png]
...
Полный прогон системы в Docker — реальная БД, реальные файлы, реальный HTTP. Без моков.
